...
export default function Workforce() {
  const ref = useReveal();
  return (
    <section id="workforce" ref={ref} className="workforce-section">
      <div className="workforce-container">
        {/* Left */}
        <div>
          <div className="reveal">
            <span className="workforce-badge">Our Team</span>
            <h2 className="workforce-heading">
              Skilled People.<br />
              <span className="workforce-heading-gradient">Strong Outcomes.</span>
            </h2>
            <p className="workforce-description">
              Our workforce is our greatest asset — qualified, trained, and committed to delivering safe, high-quality results on every project.
            </p>
          </div>
          <div className="workforce-strengths reveal rd1">
            {strengths.map(s => (
              <div key={s} className="workforce-strength">
                <div className="workforce-strength-icon">
                ...
                <span className="workforce-strength-text">{s}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Right */}
        <div className="workforce-roles">
          {roles.map((r, i) => (
            <div key={r.title} className={`reveal rd${Math.min(i + 1, 6)} workforce-role card-hover`}>
              <span className="workforce-role-icon">{r.icon}</span>
              <span className="workforce-role-badge">{r.badge}</span>
              <h4 className="workforce-role-title">{r.title}</h4>
            </div>
          ))}
        </div>
      </div>
