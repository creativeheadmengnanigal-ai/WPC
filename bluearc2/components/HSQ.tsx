...
export default function HSQ() {
  const ref = useReveal();
  return (
    <section id="hsq" ref={ref} className="hsq-section">
      <div className="hsq-container">
        <div className="hsq-header reveal">
          <span className="hsq-badge">Non-Negotiables</span>
          <h2 className="hsq-title">Health, Safety & Quality</h2>
          <p className="hsq-description">
            Safety and quality are integral to every operation — lived practices that protect our teams and deliver results clients trust.
          </p>
        </div>

        <div className="hsq-grid">
          {pillars.map((p, i) => (
            <div key={p.title} className={`reveal rd${i + 1} hsq-card card-hover`}>
              <div className="hsq-card-icon">{p.icon}</div>
              <h3 className="hsq-card-title">{p.title}</h3>
              <p className="hsq-card-desc">{p.desc}</p>
            </div>
          ))}
        </div>

        {/* Banner */}
        <div className="hsq-banner reveal">
          <div className="hsq-banner-scanline" />
          <div className="hsq-banner-content">
            <div>
              <h3 className="hsq-banner-title">Every project. Every weld. Every coat.</h3>
              <p className="hsq-banner-desc">Delivered to spec, on time, with zero safety compromises.</p>
            </div>
            <a href="#contact" className="hsq-banner-btn btn-shimmer">
              Start Your Project →
            </a>
          </div>
        </div>
